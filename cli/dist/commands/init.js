...
async function initCommand(opts) {
    ...
    try {
        ...
        if (opts.custom) {
            // ── Custom TOML config ──────────────────────────────────
            const toml = (0, config_1.loadTomlConfig)(opts.custom);
            createOpts = {
        ...
        // ── Create stablecoin ───────────────────────────────────
        const stable = await sss_token_1.SolanaStablecoin.create(connection, createOpts);
            ...
            if (toml.roles) {
                ...
                if (toml.roles.minters) {
                    for (const m of toml.roles.minters) {
                        await stable.addMinter(new web3_js_1.PublicKey(m.address), new bn_js_1.default(m.quota), authority);
                    }
                }
                // Add pausers
                if (toml.roles.pausers) {
                    for (const p of toml.roles.pausers) {
                        await stable.grantRole(sss_token_1.RoleType.Pauser, new web3_js_1.PublicKey(p), authority);
                    }
                }
                // Add burners
                if (toml.roles.burners) {
                    for (const b of toml.roles.burners) {
                        await stable.grantRole(sss_token_1.RoleType.Burner, new web3_js_1.PublicKey(b), authority);
                    }
                }
                // Add blacklisters (SSS-2 only)
                if (toml.roles.blacklisters && stable.isCompliant) {
                    for (const bl of toml.roles.blacklisters) {
                        await stable.grantRole(sss_token_1.RoleType.Blacklister, new web3_js_1.PublicKey(bl), authority);
                    }
                }
                // Add seizers (SSS-2 only)
                if (toml.roles.seizers && stable.isCompliant) {
                    for (const s of toml.roles.seizers) {
                        await stable.grantRole(sss_token_1.RoleType.Seizer, new web3_js_1.PublicKey(s), authority);
                    }
                }
        ...
        // ── Save active config ──────────────────────────────────
        const rpcUrl = (0, config_1.resolveRpcUrl)(opts.url);
        (0, config_1.saveActiveConfig)({
