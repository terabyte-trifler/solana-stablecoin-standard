...
import { statusCommand, supplyCommand, holdersCommand } from "./commands/status";
import {
  mintersListCommand,
  mintersAddCommand,
  mintersRemoveCommand,
} from "./commands/minters";
import {
  blacklistAddCommand,
  blacklistRemoveCommand,
  blacklistCheckCommand,
...
program
  ...
  .action((opts) => {
    ...
    initCommand({ ...opts, keypair: globalOpts.keypair, url: globalOpts.url });
  });
...
program
  ...
  .action((amount, opts) => {
    const g = program.opts();
    burnCommand(amount, { ...opts, keypair: g.keypair, url: g.url, config: g.config });
  });
...
program
  ...
  .action((address, opts) => {
    const g = program.opts();
    freezeCommand(address, { ...opts, keypair: g.keypair, url: g.url, config: g.config });
  });
...
program
  ...
  .action((address, opts) => {
    const g = program.opts();
    thawCommand(address, { ...opts, keypair: g.keypair, url: g.url, config: g.config });
  });
...
program
  ...
  .action((opts) => {
    const g = program.opts();
    pauseCommand({ ...opts, keypair: g.keypair, url: g.url, config: g.config });
  });
...
program
  ...
  .action((opts) => {
    const g = program.opts();
    unpauseCommand({ ...opts, keypair: g.keypair, url: g.url, config: g.config });
  });
...
program
  ...
  .action((opts) => {
    const g = program.opts();
    statusCommand({ ...opts, url: g.url, config: g.config });
  });
...
program
  ...
  .action((opts) => {
    const g = program.opts();
    supplyCommand({ ...opts, url: g.url, config: g.config });
  });
...
program
  ...
  .action((opts) => {
    const g = program.opts();
    holdersCommand({ ...opts, url: g.url, config: g.config });
  });
...
minters
  ...
  .action((opts) => {
    const g = program.opts();
    mintersListCommand({ ...opts, keypair: g.keypair, url: g.url, config: g.config });
  });
...
minters
  ...
  .action((address, opts) => {
    const g = program.opts();
    mintersAddCommand(address, { ...opts, keypair: g.keypair, url: g.url, config: g.config });
  });
...
minters
  ...
  .action((address, opts) => {
    const g = program.opts();
    mintersRemoveCommand(address, { ...opts, keypair: g.keypair, url: g.url, config: g.config });
  });
...
blacklist
  ...
  .action((address, opts) => {
    const g = program.opts();
    blacklistAddCommand(address, { ...opts, keypair: g.keypair, url: g.url, config: g.config });
  });
...
blacklist
  ...
  .action((address, opts) => {
    const g = program.opts();
    blacklistRemoveCommand(address, { ...opts, keypair: g.keypair, url: g.url, config: g.config });
  });
...
blacklist
  ...
  .action((address, opts) => {
    const g = program.opts();
    blacklistCheckCommand(address, { ...opts, keypair: g.keypair, url: g.url, config: g.config });
  });
...
blacklist
  ...
  .action((opts) => {
    const g = program.opts();
    blacklistListCommand({ ...opts, keypair: g.keypair, url: g.url, config: g.config });
  });
...
program
  ...
  .action((address, opts) => {
    const g = program.opts();
    seizeCommand(address, { ...opts, keypair: g.keypair, url: g.url, config: g.config });
  });
...
program
  ...
  .option(
    "-a, --action <type>",
    "filter by action: mint|burn|freeze|thaw|pause|blacklist|seize|role|authority"
  )
  .option("-l, --limit <count>", "max events to fetch (default: 100)")
  .option("-e, --export <file>", "export audit log to JSON file")
  .action((opts) => {
    const g = program.opts();
    auditLogCommand({ ...opts, keypair: g.keypair, url: g.url, config: g.config });
  });
