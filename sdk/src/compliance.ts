...
import { findBlacklistEntryPda, SSS_TOKEN_PROGRAM_ID } from "./pda";
...
export class ComplianceModule {
  ...
  private readonly roleManagerPda: PublicKey;
  ...
  async blacklistAdd(
    ...
    const [blacklistEntryPda] = findBlacklistEntryPda(this.configPda, address);
  ...
  async getBlacklistEntry(
    ...
    try {
      const raw = await (this.program.account as any).blacklist_entry.fetch(
        blacklistEntryPda,
      );
      return {
        stablecoin: raw.stablecoin as PublicKey,
        address: raw.address as PublicKey,
        reason: raw.reason as string,
        blacklistedAt: raw.blacklistedAt as BN,
        blacklistedBy: raw.blacklistedBy as PublicKey,
  ...
  async getAllBlacklisted(): Promise<BlacklistEntryAccount[]> {
    const accounts = await (this.program.account as any).blacklist_entry.all([
      {
        memcmp: {
    ...
    return accounts.map(({ account: raw }: { account: any }) => ({
      stablecoin: raw.stablecoin as PublicKey,
      address: raw.address as PublicKey,
      reason: raw.reason as string,
      blacklistedAt: raw.blacklistedAt as BN,
   ...
   */
  async getAuditLog(filters?: AuditLogFilters): Promise<AuditEvent[]> {
    ...
    for (const sigInfo of signatures) {
      // Apply slot filters
      if (filters?.afterSlot && sigInfo.slot <= filters.afterSlot) continue;
      if (filters?.beforeSlot && sigInfo.slot >= filters.beforeSlot) continue;
      ...
      try {
        ...
        const parsedEvents = this._parseEventsFromLogs(
          tx.meta.logMessages,
          sigInfo.signature,
          sigInfo.slot,
        ...
        for (const event of parsedEvents) {
          if (filters?.eventType && event.name !== filters.eventType) continue;
          events.push(event);
        }
      } catch {
    ...
    const serializable = events.map((e) => ({
      ...
      data: Object.fromEntries(
        Object.entries(e.data).map(([k, v]) => [
          k,
          v instanceof PublicKey ? v.toBase58() : BN.isBN(v) ? v.toString() : v,
        ]),
      ),
    }));
